...
Fixed::Fixed(const int num) { value = num << Fixed::fractBits; }

Fixed::Fixed(const float num) { value = roundf(num * (1 << Fixed::fractBits)); }
 ...
 * MEMBER FUNCTIONS
 */
int Fixed::getValue(void) const { return (value); }

void Fixed::setValue(int const new_val) { value = new_val; }

float Fixed::toFloat(void) const {
  return ((float)value / (1 << Fixed::fractBits));
}

int Fixed::toInt(void) const { return (value >> Fixed::fractBits); }

/*
 * STATIC MEMBER FUNCTIONS
 */
Fixed& Fixed::min(Fixed& fixPoint1, Fixed& fixPoint2) {
  if (fixPoint1.value < fixPoint2.value) return (fixPoint1);
  return (fixPoint2);
}
...
int Fixed::operator>(const Fixed& rhs) {
  if (this->value > rhs.getValue()) return (1);
  return (0);
}
